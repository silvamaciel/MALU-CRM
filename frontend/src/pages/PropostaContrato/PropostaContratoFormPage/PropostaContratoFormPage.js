// src/pages/PropostaContrato/PropostaContratoFormPage.js
...
function PropostaContratoFormPage() {
  ...
  // para rolar o conteúdo do wizard ao trocar de etapa
      ...
      try {
        ...
        if (isEditMode && propostaContratoId) {
          ...
          initialFormData = {
            ...proposta,
            responsavelNegociacao:
              proposta.responsavelNegociacao?._id ||
              proposta.responsavelNegociacao,
            corretagem: {
              ...proposta.corretagem,
              corretorPrincipal:
                proposta.corretagem?.corretorPrincipal?._id ||
                proposta.corretagem?.corretorPrincipal,
            },
            dataProposta: proposta.dataProposta
              ? new Date(proposta.dataProposta).toISOString().split('T')[0]
              : '',
            planoDePagamento:
              proposta.planoDePagamento?.map((p) => ({
                ...p,
                vencimentoPrimeira: p.vencimentoPrimeira
                  ? new Date(p.vencimentoPrimeira)
                    .toISOString()
                    .split('T')[0]
                  : '',
              })) || [],
          ...
          const precoImovel =
            reservaData?.imovel?.preco ||
            reservaData?.imovel?.precoTabela ||
            0;

          initialFormData = {
            ...formData,
            valorPropostaContrato:
              (formData.valorPropostaContrato ?? 0) <= 0
                ? precoImovel
            ...
            adquirentes: [
              {
                ...
                nacionalidade:
                  reservaData.lead?.nacionalidade || 'Brasileiro(a)',
            ...
            responsavelNegociacao:
              reservaData.lead?.responsavel?._id ||
              reservaData.lead?.responsavel ||
              '',
          };
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !formData.valorPropostaContrato ||
      parseFloat(formData.valorPropostaContrato) <= 0
    ) {
    ...
    const planoPagamentoValido = formData.planoDePagamento.every(
      (p) => p.valorUnitario && p.vencimentoPrimeira
    );
    ...
    switch (currentStep) {
      ...
      case 3:
        return (
          <StepCorretagem
            {...stepProps}
            brokerContactsList={brokerContactsList}
          />
        );
      case 4:
        return (
          <StepResumo {...stepProps} reservaBase={reservaBase || propostaBase} />
        );
      default:
        return <StepAdquirentes {...stepProps} />;
    }
  ...
  if ((!isEditMode && !reservaBase) || (isEditMode && !propostaBase))
    return (
      <div className="admin-page error-page">
        <p>
          Dados não encontrados. <Link to="/reservas">Voltar</Link>
        </p>
      </div>
    );
    ...
    <div className="admin-page proposta-contrato-form-page">
      ...
      <div className="page-content">
        <div className="form-container">
          ...
          {/* ÁREA QUE ROLA */}
          <div className="wizard-scroll" ref={wizardRef}>
            {renderStep()}
          </div>

          {/* Ações (grudadas embaixo do card) */}
      ...
      {showConfirmModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p>
              O valor total da proposta (entrada + parcelas) é{' '}
              <strong>
                R${' '}
                {(
                  (parseFloat(formData.valorEntrada) || 0) +
                  (formData.planoDePagamento || []).reduce(
                    (acc, p) =>
                      acc +
                      (parseFloat(p.valorUnitario) || 0) * (p.quantidade || 1),
                    0
                  )
                ...
                onClick={async () => {
                  ...
                  const dataToSubmit = {
                    ...
                    modeloContratoUtilizado:
                      formData.modeloContratoUtilizado || null,
                    ...
                    valorEntrada: formData.valorEntrada
                      ...
                      : null,
                    planoDePagamento: formData.planoDePagamento.filter(
                      (p) => p.valorUnitario && p.vencimentoPrimeira
                    ),
                    corretagem: formData.corretagem?.valorCorretagem
                      ? {
                        ...formData.corretagem,
                        valorCorretagem:
                          Number(formData.corretagem.valorCorretagem) || 0,
                      }
                      : null,
                  };
                  ...
                  try {
                    let result;
                    if (isEditMode) {
                      result = await updatePropostaContratoApi(
                        propostaContratoId,
                        dataToSubmit
                      );
                      toast.success(
                        'Proposta/Contrato atualizada com sucesso!'
                      );
                    } else {
                      result = await createPropostaContratoApi(
                        reservaId,
                        dataToSubmit
                      );
                      toast.success(
                        'Proposta/Contrato criada com sucesso!'
                      );
                    }
                    navigate(
                      `/propostas-contratos/${result._id || propostaContratoId}`
                    );
                  } catch (err) {
                    const errMsg =
                      err.error ||
                      err.message ||
                      'Erro ao salvar Proposta/Contrato.';
                    setFormError(errMsg);
                    toast.error(errMsg);
                  } finally {
                    setIsSaving(false);
