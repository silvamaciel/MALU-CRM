...
function PropostaContratoFormPage() {
    ...
    const proceedSubmit = async (dataToSubmit) => {
        try {
            let result;
            if (isEditMode) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (dataToSubmit.valorPropostaContrato < precoTabela) {
            setPendingSubmitData(dataToSubmit);
            setShowConfirmModal(true);
        } else {
            await proceedSubmit(dataToSubmit);
        }
    };
            ...
            {showConfirmModal && (
                <div className="modal-overlay">
                    <div className="modal-content">
                        <h2>Valor Inferior ao Tabela</h2>
                        <p>O valor da proposta ({formData.valorPropostaContrato}) está abaixo do valor de tabela ({precoTabela}). Deseja continuar?</p>
                        <div className="modal-buttons">
                            <button onClick={() => proceedSubmit(pendingSubmitData)} className="button primary-button">Sim, continuar</button>
                            <button onClick={() => {
                                setShowConfirmModal(false);
                                setPendingSubmitData(null);
                                setIsSaving(false);
                            }} className="button cancel-button">Cancelar</button>
                        </div>
                    </div>
                </div>
            )}
