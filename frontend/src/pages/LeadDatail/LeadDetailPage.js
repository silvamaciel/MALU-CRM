...
function LeadDetailPage() {
  ...
  const handleConfirmDiscard = useCallback(
    ...
    [leadDetails, handleCloseDiscardModal, forceRefresh]
  ...
  const handleReactivateLead = useCallback(async () => {
    ...
    setReactivateError(null); // Limpa erro anterior
    try {
      const leadReativado = await updateLead(leadDetails._id, {
        situacao: situacaoAtendimento._id,
      });
  ...
  const handleOpenReservaModal = () => {
    if (
      leadDetails &&
      (leadDetails.situacao?.nome === "Vendido" ||
        leadDetails.situacao?.nome === "Descartado" ||
        leadDetails.situacao?.nome === "Em Reserva")
    ) {
      toast.warn(
        `Lead com status "${leadDetails.situacao.nome}" não pode ter nova reserva neste momento.`
      );
      return;
    }
  ...
  if (!leadDetails) {
    return (
      <div className="lead-detail-page error-page">Lead não encontrado.</div>
    );
  }
      ...
      <div className="detail-header">
        ...
        <div className="header-actions">
          ...
          {!isCurrentlyDiscarded && (
            <Link to={`/leads/${id}/editar`} className="button edit-button">
              Editar
            </Link>
          )}
          {isCurrentlyDiscarded ? (
          ...
          {!isCurrentlyDiscarded &&
            leadDetails.situacao?.nome !== "Em Reserva" &&
            leadDetails.situacao?.nome !== "Proposta" &&
            leadDetails.situacao?.nome !== "Vendido" && ( // <<< CONDIÇÃO PARA MOSTRAR O BOTÃO
              <button
                onClick={handleOpenReservaModal}
                className="button action-button" // Ou um estilo específico como "reserve-button"
                style={{ marginLeft: "10px" }} // Adiciona margem se necessário
              >
                Criar Reserva
              </button>
            )}
          ...
          <div className="detail-grid">
            ...
            <div className="detail-item">
              ...
              <span className="detail-value">
                {leadDetails.contato || "N/I"}
              </span>
            </div>
            {/* CPF */}
            <div className="detail-item">
              <span className="detail-label">CPF:</span>
              <span className="detail-value">{leadDetails.cpf || "N/I"}</span>
            </div>
            {/* Nascimento */}
            <div className="detail-item">
            ...
            {/* Criação */}
            <div className="detail-item">
              <span className="detail-label">Criação:</span>
              <span className="detail-value">
            ...
            {/* Atualização */}
            <div className="detail-item">
              <span className="detail-label">Atualização:</span>
              <span className="detail-value">
