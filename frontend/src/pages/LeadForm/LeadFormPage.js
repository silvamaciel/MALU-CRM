...
function LeadFormPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(
        `Erro ao ${isEditMode ? "atualizar" : "cadastrar"} lead:`,
        err
      );
      toast.error(
        err.message ||
          `Falha ao ${isEditMode ? "atualizar" : "cadastrar"} o lead.`
      );
    } finally {
      setIsProcessing(false);
    }
  ...
  return (
    <div className="lead-form-page">
      <h1>
        {isEditMode ? `Editar Lead: ${formData.nome}` : "Cadastrar Novo Lead"}
      </h1>
