...
const logHistory = async (leadId, userId, action, details) => {
  try {
    ...
    console.log(
      `[History] Logged: Lead ${leadId}, Action: ${action}, User: ${userId || "System"
      }`
    );
  } catch (error) {
    console.error(
...
const getLeads = async (queryParams = {}, companyId) => {
  ...
  const stageParam = queryParams.situacao || queryParams.leadstage || queryParams.leadStage;

  if (stageParam) {
    const ids = String(stageParam)
      .split(',')
      .map(s => s.trim())
    ...
    if (ids.length === 1) {
      queryConditions.situacao = ids[0];
    } else if (ids.length > 1) {
      queryConditions.situacao = { $in: ids };
    }
  }
