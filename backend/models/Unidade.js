...
const unidadeSchema = new Schema({
    ...
    statusUnidade: {
        ...
        enum: {
            values: ["Disponível", "Reservada", "Proposta Aceita", "Vendido", "Bloqueado"],
...
unidadeSchema.pre('save', async function(next) {
    if (this.isNew || this.isModified('empreendimento')) {
        try {
            ...
            const emp = await EmpreendimentoModel.findById(this.empreendimento).select('company').lean();
            if (emp) {
                this.company = emp.company;
            } else {
                next(new Error('Empreendimento pai não encontrado para definir o companyId da unidade.'));
                return;
            }
        } catch (error) {
            next(error);
